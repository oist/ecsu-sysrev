...
import json
import requests
from key import ELSAPI_API_KEY

CONFIG = {
    "apikey": ELSAPI_API_KEY,
    "insttoken": ""
}
...
def get_scopus_info(SCOPUS_ID):
    ...
    resp = requests.get(url,
                    headers={'Accept':'application/json',
                             'X-ELS-APIKey': ELSAPI_API_KEY})

    return json.loads(resp.text.encode('utf-8'))

if __name__ == '__main__':
	# syntax https://dev.elsevier.com/tips/ScopusSearchTips.htm
	client = ElsClient(CONFIG['apikey'])
	doc_srch = ElsSearch('active+perception','scopus')
	doc_srch.execute(client, get_all = False)
	results = doc_srch.results
	r = results[0]
	id = r["dc:identifier"].split(':')[1] # "SCOPUS_ID:85078097723"
	# print(get_scopus_info(id))
	print(json.dumps(r, indent=3, ensure_ascii=False))
